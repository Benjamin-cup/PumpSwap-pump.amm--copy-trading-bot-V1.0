...
import { RPC_ENDPOINT, PRIVATE_KEY, SOL_MINT, SOL_DECIMAL } from '../../constants';
...
export async function fetchMarketId(connection: Connection, baseMint: PublicKey, quoteMint: PublicKey) {
  try {
    ...
    console.log("marketid2", accounts.map(({ account }) => MARKET_STATE_LAYOUT_V3.decode(account.data))[0]);
...
export async function sellderivePoolKeys(
  poolId: PublicKey,
  marketId: PublicKey,
  baseMint: PublicKey,
  tokenDecimal: number): Promise<LiquidityPoolKeysV4 | null> {
  const marketInfo = await getMarketInfo(marketId);
  if (!marketInfo) return null;
  logger.info('marketInfo.data', marketInfo.data);

  const marketDeco = await getDecodedData(marketInfo);
  const baseDecimals = tokenDecimal;
  const ownerBaseAta = await getOwnerAta(baseMint, wallet.publicKey);
  const quoteMint = new PublicKey(SOL_MINT);
  const quoteDecimals = SOL_DECIMAL;
...
export async function buyDerivePoolKeys(
  poolId: PublicKey,
  marketId: PublicKey,
  baseMint: PublicKey,
  tokenDecimal: number
  ...
  const poolKeys = {
    ...
    marketBids: marketDeco.bids,
    marketAsks: marketDeco.asks,
    marketQuoteVault: ownerQuoteAta,
    marketBaseVault: ownerBaseAta,
    marketEventQueue: marketDeco.eventQueue,
    id: PublicKey.findProgramAddressSync(
      [RayLiqPoolv4.toBuffer(), marketId.toBuffer(), Buffer.from('amm_associated_seed', 'utf-8')],
      RayLiqPoolv4
...
export async function getPoolID(tokenaddress: string, connection: Connection) {
  let base = new PublicKey(tokenaddress);
  const quote = new PublicKey(spl.NATIVE_MINT);
  const commitment = "confirmed";

  try {

    // First try with base
    const baseAccounts = await connection.getProgramAccounts(new PublicKey(RayLiqPoolv4), {
      commitment,
      filters: [
    ...
    const quoteAccounts = await connection.getProgramAccounts(new PublicKey(RayLiqPoolv4), {
      commitment,
      filters: [
        { dataSize: LIQUIDITY_STATE_LAYOUT_V4.span },
        {
...
export async function makeDerivePoolKeys(
  poolId: PublicKey,
  poolState: any,
  baseMint: PublicKey,
  tokenDecimal: number
): Promise<LiquidityPoolKeysV4 | null> {
  const marketInfo = await getMarketInfo(new PublicKey(poolState.marketId));
  if (!marketInfo) return null;
  logger.info('marketInfo.data', marketInfo.data);
  const marketId = new PublicKey(poolState.marketId);
  const baseDecimals = tokenDecimal;
  const ownerBaseAta = await getOwnerAta(baseMint, wallet.publicKey);
  const quoteMint = new PublicKey(SOL_MINT);
  const quoteDecimals = SOL_DECIMAL;
  ...
  const version: 4 | 5 = 4
  const marketVersion: 3 = 3
  const marketAuthority = new PublicKey(poolState.marketAuthority);
  ...
  const poolKeys = {
    ...
    marketProgramId: openbookProgram,
    marketId,
    marketBids: new PublicKey(poolState.marketBids),
    marketAsks: new PublicKey(poolState.marketAsks),
    marketQuoteVault: new PublicKey(poolState.marketQuoteVault),
    marketBaseVault: ownerBaseAta,
    marketEventQueue: ownerQuoteAta,
    id: PublicKey.findProgramAddressSync(
      [RayLiqPoolv4.toBuffer(), marketId.toBuffer(), Buffer.from('amm_associated_seed', 'utf-8')],
      RayLiqPoolv4
    )[0],
